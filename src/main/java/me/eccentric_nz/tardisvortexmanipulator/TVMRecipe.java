...
public class TVMRecipe {
    ...
    public ShapedRecipe makeRecipe() {
        String[] result_iddata = Objects.requireNonNull(plugin.getConfig().getString("recipe.result")).split(":");
        ...
        if (!Objects.equals(plugin.getConfig().getString("recipe.lore"), "")) {
            im.setLore(Arrays.asList(Objects.requireNonNull(plugin.getConfig().getString("recipe.lore")).split("~")));
        }
        ...
        // get shape
        try {
            String[] shape_tmp = Objects.requireNonNull(plugin.getConfig().getString("recipe.shape")).split(",");
