	...
	public void onInteract(PlayerInteractEvent event) {
		...
		if (is.hasItemMeta() && Objects.requireNonNull(is.getItemMeta()).hasDisplayName() &&
			...
			if (rs.resultSet()) {
				if (action.equals(Action.RIGHT_CLICK_AIR)) {
				...
				} else if (plugin.getConfig().getBoolean("allow.look_at_block") &&
						   player.hasPermission("vm.lookatblock")) {
					...
					if (!TVMUtils.checkTachyonLevel(uuid.toString(), actual)) {
						player.sendMessage(
								plugin.getPluginName() + "You need at least " + actual + " tachyons to travel!");
						return;
					}
					...
					if (rnd.nextInt(100) < plugin.getConfig().getInt("block_travel_malfunction_chance")) {
						...
						if (_bl != null) {
							player.sendMessage(plugin.getPluginName() +
											   "Vortex travel malfunction. Attempting to land in safe location..");
							bl = _bl;
						}
					}
